...
export default function Stats({ gameStat }: any) {
		...
		async function fetchGameHistory() {
			try {
				const response = await axios.get(
					`http://${appHostUrl}:${backendPort}/users/general-information`,
					{
						headers: {
				...
				const index = filteredData.findIndex(ranked => (ranked?.authId === user?.authId));
